...
import time
from data.eval_data_reader_cv2_py3 import load_bin
...
def get_parser():
    ...
    parser.add_argument('--num_output', default=2000, help='the image size')
    ...
    parser.add_argument('--summary_path', default='./output/summary/cpu', help='the summary file save path')
    parser.add_argument('--ckpt_path', default='./output/ckpt/cpu', help='the ckpt file save path')
    ...
    parser.add_argument('--validate_interval', default=200, help='intervals to save ckpt file')
...
if __name__ == '__main__':
    ...
    variables_to_restore = [v for v in variables if v.name.split('/')[0]!='arcface_loss']
    
    restore_saver = tf.train.Saver(variables_to_restore)
    restore_saver.restore(sess, 'output/ckpt/multi/InsightFace_iter_170000.ckpt')
